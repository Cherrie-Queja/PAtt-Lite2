...
class Affectdataset(data.Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, basic_aug=False, download=False, small=''):
        self.root = root
        ...
        NAME_COLUMN = 0
        LABEL_COLUMN = 1
        df_train = pd.read_csv(os.path.join(self.root, f'train_set/train_annotations_7class{small}.txt'), sep=' ',
                               header=None)
        df_valid = pd.read_csv(os.path.join(self.root, f'valid_set/valid_annotations_7class{small}.txt'), sep=' ',
                               header=None)
        if self.train:
            dataset = df_train
        else:
...
# path = ("/home/panr/all_datasets/AffectNet/train_set/annotations")
# files = sorted(glob.glob(path + '/*_exp.npy'))
# id_file = []
# label = []
# for i in range(len(files)):
#     if np.load(files[i]).astype(int) < 7:
#         id_file.append(files[i][56:-8])
#         label.append(np.array(np.load(files[i])).tolist())
# print(len(files))
# print("af", len(label))
#
# with open('/home/panr/all_datasets/AffectNet/train_set/train_annotations_7class.txt', 'w+') as f:
...
# path = ("/home/panr/all_datasets/AffectNet/val_set/annotations")
# files = sorted(glob.glob(path + '/*_exp.npy'))
# id_file = []
# label = []
# for i in range(len(files)):
#     if np.load(files[i]).astype(int) < 8:
#         id_file.append(files[i][54:-8])
#         label.append(np.array(np.load(files[i])).tolist())
# print(len(files))
# print("af", len(label))
#
# with open('/home/panr/all_datasets/AffectNet/val_set/valid_annotations_8class.txt', 'w+') as f:
#     for i in range(len(id_file)):
#         # f.write("%s\n" % item)
#         f.write("%s.jpg %s\n" % (id_file[i], label[i]))
#     f.close()
