...
class Backbone(Module):
    def __init__(self, num_layers, drop_ratio, mode='ir', with_head=True):
        super(Backbone, self).__init__()
        # assert num_layers in [50, 100, 152], 'num_layers should be 50,100, or 152'
        assert mode in ['ir', 'ir_se'], 'mode should be ir or ir_se'
        blocks1, blocks2, blocks3, blocks4 = get_blocks(num_layers)
        ...
        if with_head:
            self.body4 = Sequential(*modules4)  # panr
    ...
    def forward(self, x):
        ...
        x1 = self.body1(x)  # (b,64,56,56)
        x2 = self.body2(x1)  # (b,128,28,28)
        x3 = self.body3(x2)  # (b,256,14,14)
        if self.with_head:
            x4 = self.body4(x3)  # (b,512,7,7)
